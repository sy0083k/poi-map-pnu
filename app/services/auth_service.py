...
def login(request: Request, username: str, password: str, csrf_token: str) -> JSONResponse:
    ...
    try:
        ...
        logger.error("invalid admin password hash configuration", extra={"request_id": request_id})
        return JSONResponse(
            status_code=500,
            content={"success": False, "message": "서버 인증 설정 오류입니다. 관리자에게 문의하세요."},
        )
    except Exception:
        logger.exception("password backend verification failed", extra={"request_id": request_id})
        return JSONResponse(
            status_code=500,
