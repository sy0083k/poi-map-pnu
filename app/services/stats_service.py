...
def record_map_event(payload: dict[str, Any]) -> None:
    ...
    raw_payload_json = _serialize_raw_payload(payload)
    ...
    if event_type == EVENT_TYPE_SEARCH:
        ...
        with db_connection() as conn:
            ...
            idle_land_repository.insert_raw_query_log(
                ...
                event_type=EVENT_TYPE_SEARCH,
                anon_id=anon_id,
                raw_region_query=_normalize_raw_text(
                    payload.get("rawSearchTerm", payload.get("searchTerm")), max_length=1000
                ),
                raw_min_area_input=_normalize_raw_text(
                    payload.get("rawMinAreaInput", payload.get("minArea")), max_length=1000
                ),
                raw_max_area_input=_normalize_raw_text(payload.get("rawMaxAreaInput"), max_length=1000),
                raw_rent_only_input=_normalize_raw_text(payload.get("rawRentOnly"), max_length=32),
    ...
    if event_type == EVENT_TYPE_LAND_CLICK:
        ...
        with db_connection() as conn:
            ...
            idle_land_repository.insert_raw_query_log(
                ...
                raw_land_id_input=_normalize_raw_text(payload.get("landId"), max_length=64),
                raw_land_address_input=_normalize_raw_text(payload.get("landAddress"), max_length=1000),
                raw_click_source_input=_normalize_raw_text(payload.get("clickSource"), max_length=32),
                raw_payload_json=raw_payload_json,
            )
            conn.commit()
        return
...
def export_raw_query_csv(
    *,
    ...
    created_at_from = _parse_date_start(date_from)
    created_at_to = _parse_date_end_exclusive(date_to)
    ...
    with db_connection(row_factory=True) as conn:
        rows = idle_land_repository.fetch_raw_query_logs(
            conn,
