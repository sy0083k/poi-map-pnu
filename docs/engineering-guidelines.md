...
- External API/Data: VWorld (WMTS), FlatGeobuf (`data/*.fgb`)
...
- `MUST`: 비밀값(`SECRET_KEY`, 비밀번호 해시 등)은 로그/응답에 노출하지 않는다. 단, 공개 클라이언트 렌더링에 필수인 공개용 키(`VWORLD_WMTS_KEY`)는 예외적으로 응답 노출을 허용하되, 최소 권한(도메인/용도 제한)과 사용량 모니터링 정책을 유지한다.
