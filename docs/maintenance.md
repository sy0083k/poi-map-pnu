...
- 라이트박스에서 마우스 휠 확대/축소 및 확대 상태 드래그 이동이 동작하고, 닫기 후 재열기 시 배율/위치가 초기화되는지 점검
- 세로 긴 사진에서 라이트박스 드래그 팬 이동량이 충분한지(캡션 영역과 분리된 이미지 뷰포트 기준) 점검
- 라이트박스 확대 시 이미지가 흐릿하게 보이지 않는지(저해상도 합성 캐시 징후) 확인
- 드래그 중에는 넓게 이동되고, 마우스 버튼 해제 시 경계 안으로 부드럽게 정렬되는지(120ms) 확인
- `Shift + 드래그` 고속 팬이 동작하는지 확인
- 드래그 중 포인터가 라이트박스 뷰포트 가장자리 근처에 있을 때 자동 팬(연속 이동)이 동작하는지 확인
- 확대 후 축소(휠/`-`/`0`) 시 이미지가 항상 중앙 원위치로 정규화되는지 확인
- `/photo2map`에서 마커 생성 후 `/file2map` 이동-복귀 시 저장된 사진 마커가 자동 복원되는지 점검
- `/file2map`에서 저장된 사진 마커가 지도에 표시되고, 마커 클릭 시 우하단 선택 사진 패널/대형 모달이 동작하는지 점검
