...
- `/siyu`에서 검색 결과 요약 바(`#results-summary-bar`)가 결과 건수, 적용된 읽기 전용 필터 조건, 조건 초기화/다운로드 액션, 로딩/빈 결과/차단/오류 상태 메시지를 표시하는지 점검
- `/siyu`에서 검색 결과가 있는 ready 상태의 요약 바가 compact 표시로 전환되어 검색 목록 영역에 최소 5건이 보일 공간을 확보하는지 점검
- `/siyu` 검색 목록 row가 compact 높이로 표시되면서 최소 48px 터치/클릭 영역과 텍스트 가독성을 유지하는지 점검
- `/siyu` 검색 목록 row가 키보드 포커스 가능한 버튼으로 동작하고, 선택 row가 `aria-selected`와 `선택됨` 배지를 함께 제공하는지 점검
...
- `/file2map`에서 검색 결과 요약 바가 업로드 미적용/0건 상태에서는 다운로드 불가 사유와 차단/빈 결과 메시지를 표시하고, 업로드 후 검색 결과 건수와 적용된 필터 조건 및 다운로드 액션을 표시하는지 점검
...
- 모바일 결과 상태에서 검색 결과 요약 바가 바텀시트 상단에 표시되고, 긴 필터 칩 값이 내부에서 줄바꿈/스크롤되어 목록과 겹치지 않는지 점검
