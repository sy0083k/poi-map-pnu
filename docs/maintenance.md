...
- `주제도` 하위 메뉴(`시유지/파일→주제도`) 노출 점검
- `주제도`에서 `파일→주제도` 클릭 시 `/gukgongyu`로 이동하고, `시유지` 클릭 시 테마 전환과 활성 메뉴(`is-active`)가 즉시 갱신되는지 점검
- 주제도 전환 시 URL이 `국·공유지=/gukgongyu`, `시유지=/siyu`로 동기화되고, 직접 URL 진입/새로고침/브라우저 뒤로가기에 테마가 일치하는지 점검
- 루트(`/`) 접속 시 `307`으로 `/siyu`로 이동하는지 점검
- 유틸리티 사이드바에서 `재산용도` 콤보(`전체/행정재산/일반재산`)와 `지목` 입력이 데스크톱/모바일 모두 표시되고 검색 조건으로 반영되는지 점검
- 필터 배치가 `지역명·주소 검색` 바로 아래 `재산용도/지목` 1줄, 그 다음 `면적 직접 입력` 순서인지 점검
- `/siyu`, `/gukgongyu` 모두에서 `재산관리관` 입력이 표시되고 동일하게 검색 조건으로 반영되는지 점검
...
- `/gukgongyu` 초기 동등화가 필요할 때 `python scripts/clone_city_data_to_national.py --dry-run`으로 사전 건수 확인 후 복제를 실행했는지 점검
