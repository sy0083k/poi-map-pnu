...
- `/siyu`에서 검색 결과 요약 바(`#results-summary-bar`)가 결과 건수, 적용된 읽기 전용 필터 조건, 다운로드 가능 여부를 표시하는지 점검
...
- `/file2map`에서 검색 결과 요약 바가 업로드 미적용/0건 상태에서는 다운로드 불가를 표시하고, 업로드 후 검색 결과 건수와 적용된 필터 조건을 표시하는지 점검
