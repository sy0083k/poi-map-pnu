...
- `/siyu` 초기 진입 시 `/api/lands/list`가 `theme + filter` 전체 결과를 cursor 끝까지 읽어 목록을 구성하는지 점검
- `/api/lands/list` 응답의 `totalCount`가 `bbox`를 제외한 현재 `theme + filter + geom 존재(parcel_render_item)` 기준 전체 건수를 반환하는지 점검
- `/api/lands/list` 응답 item에 `bbox[minx,miny,maxx,maxy]`가 포함되고, 값이 `CADASTRAL_FGB_CRS` 기준으로 해석되는지 점검
- `/siyu` 사이드바에서 `목록 더 불러오기`가 노출되지 않는지 점검
...
- `/siyu`가 전체 목록을 유지하되 지도 캔버스에서는 현재 화면 context bbox와 목록 item `bbox` 교집합만 렌더하는지 점검
