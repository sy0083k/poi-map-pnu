...
- `/photo2map` 기능: 공통 지도 셸(`index.html`)에서 `photo` 모드로 동작, 폴더 선택(`webkitdirectory`) 기반 JPEG EXIF GPS 파싱 후 지도 마커 렌더, 데스크톱 우측 사이드바 목록/네비게이션 + 지도 우하단 미리보기 패널 연동(클릭 시 대형 모달 확대), `/file2map` 로컬 업로드 토지 하이라이트 재사용 및 토지 클릭 상세정보 표시
