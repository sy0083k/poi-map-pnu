...
- `/photo2map` 기능: 공통 지도 셸(`index.html`)에서 `photo` 모드로 동작, 폴더 선택(`webkitdirectory`) 기반 JPEG EXIF GPS 파싱 후 지도 마커 렌더, 데스크톱 좌측(기본) 사이드바 목록/네비게이션 + 지도 우하단 미리보기 패널 연동(클릭 시 새창 열기), `/file2map` 로컬 업로드 토지 하이라이트 재사용 및 토지 클릭 상세정보 표시
...
- `/file2map` 사진 연동: 저장된 사진 마커를 지도에 함께 렌더하고, 마커 클릭 시 우하단 선택 사진 패널(클릭 시 새창) 표시
- `/siyu` 사진 연동: 저장된 사진 마커가 있을 경우 동일한 선택 사진 패널(클릭 시 새창)과 상세 패널 비가림 정책을 적용
