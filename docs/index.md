...
- 상세 정보 UI: 선택 필지 정보를 우상단 패널의 2열(속성/값) 동적 필드(`sourceFields`)로 동일 라인 정렬해 표시하며, `X` 버튼으로 닫기 지원. `/siyu`는 지도 클릭 시에만 패널을 연다.
- 상세 패널 제목 정책: `/siyu`는 `재산 상세 정보`, `/file2map`·`/photo2map`은 `상세 정보`
- 상세 패널 초기 상태: 웹앱 초기화 시 숨김. `/siyu`는 지도 클릭 시에만 자동 표시되고, 목록 클릭/이전·다음 네비게이션은 패널을 열지 않는다.
