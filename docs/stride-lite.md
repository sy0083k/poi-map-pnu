...
- 위험: 관리자 화면 공개용 Geocoder 키(`VWORLD_GEOCODER_KEY`) 유출(세션 탈취/내부자 오남용/화면 캡처 등).
  - 현재: 관리자 보호 화면(`/admin`)에서 운영 목적의 예외 공개를 허용.
  - 완화: 관리자 경로 접근 통제(내부망/세션, 상태 변경 요청 CSRF), 키 사용량 모니터링, 주기적 로테이션 및 유출 대응 런북 유지.
