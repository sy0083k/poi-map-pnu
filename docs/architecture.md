...
2. 테마별 목록 소스를 결정한다.
   ...
   - `/siyu`와 `/file2map` 유틸리티 사이드바는 필터 영역과 목록 사이에 검색 결과 요약 바(`#results-summary-bar`)를 표시한다. 요약 바는 결과 건수, 적용된 읽기 전용 필터 조건, 다운로드 가능 여부를 보여주며, 초기/0건/업로드 미적용 상태에서는 다운로드 불가로 표시한다.
