...
6. 마커/목록/네비게이션 선택은 단일 선택 상태를 공유하며 지도 우하단 미리보기 패널 이미지를 갱신한다.
   - 미리보기 패널 클릭 시 Viewer.js 라이트박스를 열고 확대/축소/팬/이전·다음 사진 이동을 지원한다.
...
8. `/file2map`·`/siyu` 진입 시 저장된 사진 마커가 있으면 지도에 함께 표시되고, 마커 클릭 시 우하단 선택 사진 패널(클릭 시 라이트박스 열기)을 동일하게 제공한다.
