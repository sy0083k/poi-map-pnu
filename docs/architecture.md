...
11. 상단 헤더는 `시작`(같은 창 `/readme`), `배경지도`, `주제도` 메뉴를 제공한다.
    ...
    - `시유지(/siyu)`와 `파일→지도(/file2map)`는 동일한 필터 집합(주소/재산용도/지목/면적/재산관리관)으로 목록을 필터링한다.
    - 재산관리관 조건 검색에서 필터 결과의 고유 재산관리관 값이 2개 이상이면 검색을 중단하고 상태 영역(`#map-status`, 지도 캔버스 상단 1줄 오버레이)에 다중 검출 목록을 표시한다.
    - `/file2map` 초기 동등화 데이터는 운영 배치 스크립트(`scripts/clone_city_data_to_national.py`)로 `poi_city -> poi` 1회 복제한다.
