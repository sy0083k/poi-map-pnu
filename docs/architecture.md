...
6. 마커/목록/네비게이션 선택은 단일 선택 상태를 공유하며 지도 우하단 미리보기 패널 이미지를 갱신한다.
   - 미리보기 패널 클릭 시 지도 위 대형 모달 패널을 열어 확대 표시한다(ESC/배경 클릭/닫기 버튼 닫기).
...
7. `/file2map` 업로드 데이터가 브라우저에 저장돼 있으면 동일 PNU 하이라이트를 `/photo2map`에도 표시하고, 토지 클릭 시 우상단 상세 정보 패널(`source_fields`)을 렌더링한다.
   - 상세 정보 패널은 `photo2map` 모드에서 가용 높이를 동적으로 제한해(뷰포트/상단 바/우하단 사진 패널 여유 높이 반영) 두 패널 동시 노출 시 겹침을 방지한다.
8. `/file2map` 진입 시 저장된 사진 마커가 있으면 지도에 함께 표시되고, 마커 클릭 시 우하단 선택 사진 패널/대형 모달을 동일하게 제공한다.
   - 선택 사진 패널이 열려 있는 동안 `/file2map` 상세 정보 패널 최대 높이를 안전 영역으로 제한해 두 패널의 겹침을 방지한다.
