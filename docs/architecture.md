...
2. 테마별 목록 소스를 결정한다.
   ...
   - 유틸리티 사이드바의 토지 목록 row는 가상화 pool을 유지하되 `button` 기반 선택 항목으로 렌더링하며, 현재 선택 row는 `aria-selected`와 `선택됨` 배지로 전달한다.
   - `/siyu`와 `/file2map` 유틸리티 사이드바는 필터 영역과 목록 사이에 검색 결과 요약 바(`#results-summary-bar`)를 표시한다. 요약 바는 결과 건수, 적용된 읽기 전용 필터 조건, 조건 초기화/다운로드 액션, 로딩/빈 결과/차단/오류 상태 메시지를 보여주며, 초기/0건/업로드 미적용 상태에서는 다운로드 불가 사유를 함께 표시한다.
