...
2. 테마별 목록 소스를 결정한다.
   - `/siyu(city_owned)`: `/api/lands/list?theme=city_owned`로 목록을 조회한다.
     - 초기 진입과 필터 실행은 cursor 기반으로 전체 결과를 끝까지 읽어 사이드바 목록을 구성한다.
     - 각 item은 `parcel_render_item` 기반 bbox 메타를 포함하며 좌표계는 `CADASTRAL_FGB_CRS`를 따른다. 프런트는 화면 bbox를 같은 CRS로 변환해 교집합 렌더 집합만 따로 계산한다.
...
3. 클라이언트는 목록 PNU를 대상으로 하이라이트를 구성한다.
   ...
   - `/siyu`는 전체 목록 중 현재 extent를 1.5배 확장한 bbox와 교차하는 항목만 render PNU 집합으로 추려 하이라이트를 요청한다.
   ...
   - `/siyu`에서 화면 밖 목록 항목을 클릭하면 현재 bbox 안 토지들과 선택 필지 1건을 함께 하이라이트해 목록 선택과 지도 컨텍스트를 동시에 유지한다.
   ...
   - `render_grid_cell`/`render_grid_parcel`은 고정 격자 기반 후보 축소 인덱스이며, 현재는 단일 `grid_level=0`만 사용한다.
...
7. 지도 엔진별 투영 정책을 유지한다.
   - `/api/cadastral/highlights` 응답은 `items[{pnu, geometry, lod, bbox, center}]` 최소 구조를 반환하고 `meta.responseCrs`는 `CADASTRAL_FGB_CRS`와 동일하다. bbox 요청에는 `meta.gridApplied/gridCellCount/gridCandidatePnuCount/fallbackUsed` 관측 필드가 추가된다.
