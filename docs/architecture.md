...
11. 상단 헤더는 `시작`(같은 창 `/readme`), `배경지도`, `주제도` 메뉴를 제공한다.
    ...
    - `주제도` 메뉴는 `시유지`, `파일→주제도` 하위 항목을 제공한다.
    - `파일→주제도`는 `http://127.0.0.1:8000/gukgongyu`(내부 경로 `/gukgongyu`)로 이동한다.
    ...
    - 유틸리티 사이드바 필터는 공통으로 주소/재산용도/지목/면적 조건을 지원한다.
    - `재산용도`는 콤보박스(`전체/행정재산/일반재산`)이며, 선택값은 원본 컬럼 `재산용도`와 완전 일치로 필터링한다.
    - `시유지(/siyu)`와 `파일→주제도(/gukgongyu)`는 동일한 필터 집합(주소/재산용도/지목/면적/재산관리관)으로 목록을 필터링한다.
    - 재산관리관 조건 검색에서 필터 결과의 고유 재산관리관 값이 2개 이상이면 검색을 중단하고 상태 영역(`#map-status`, 지도 캔버스 상단 1줄 오버레이)에 다중 검출 목록을 표시한다.
    - `/gukgongyu` 초기 동등화 데이터는 운영 배치 스크립트(`scripts/clone_city_data_to_national.py`)로 `poi_city -> poi` 1회 복제한다.
